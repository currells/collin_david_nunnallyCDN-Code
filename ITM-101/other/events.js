...
function taskTemplate(task) {
  ...
  <li class="${task.completed ? "strike" : ""}">
    ...
    <div>
      ...
      <span data-function="completed">✅</span>
    </div>
  </li>`;
}
...
function manageTasks(event) {
    ...
    if (!taskElement) return; // Ensure we clicked inside a task element
    
    // Check which function to call based on the `data-function` attribute
    if (event.target.dataset.function === "delete") {
      removeTask(taskElement);
    } else if (event.target.dataset.function === "completed") {
