def main():
    ...
    if vin in vehicles_dict:
        details = vehicles_dict[vin]
        manufacturer = details[MANUFACTURER_INDEX]
        model = details[MODEL_INDEX]
        color = details[COLOR_INDEX]
        print(f"Vin: {vin}, Manufacturer: {manufacturer}, Model: {model}, Color: {color}")
