...
Class: CSE-111
Comments:
Code copied from heart_rate.py and modified to do what is required
"""
...
def populate_main_window(frm_main):
    ...
    def calculate(event):
        ...
        """
        try:
            radius = ent_radius.get() #Gets the input from the radius box
            area = math.pi * (radius ** 2) # Computes the area

            lbl_area.config(text=f"{area:.2f}") # Displays the area

        except ValueError:
             # When the user deletes all the digits in the radius entry box, clear the area.
    ...
    # Bind the calculate function to the radius entry box so that the computer will call the calculate function when the user changes the text in the entry box.
